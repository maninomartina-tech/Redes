...
export default function PlanningBoard() {
  ...
  return (
    <div>
      ...
      {/* Cómo mirarlo */}
      <div className="mb-4 inline-flex rounded-xl border border-ink-200 bg-surface p-1">
      ...
      {vista === 'calendario' ? (
        ...
        <Etapas posts={clientPosts} onOpen={setSelected} />
      )}
 ...
 */
function Etapas({ posts, onOpen }: { posts: Post[]; onOpen: (id: string) => void }) {
  ...
  return (
    <div className="w-72 shrink-0">
      ...
      <div className="space-y-3 rounded-2xl bg-ink-100/50 p-2.5">
        ...
        {items.map((p) => (
          <PostCard key={p.id} post={p} onClick={() => onOpen(p.id)} />
        ))}
      </div>
    </div>
  );
